...
- [ ] `POST /auth/register` — **회원가입 3단계 순서 준수**
      이름·이메일·비밀번호 입력 → 유형 선택 → 고령자라면 초대 코드 입력/건너뛰기 → 그때 `role`과 함께 1회 호출
      (첫 화면 입력만으로 계정을 만들지 않는다. 중간 값은 메모리에만 임시 보관)
- [ ] `POST /auth/login`
- [x] `POST /auth/oauth/{kakao|naver}/prepare`·`/complete` — provider 인증 후 기존 계정은 저장된 역할로 진입하고 신규 계정만 역할 선택 (#56)
      Expo Web은 `auth/callback/:provider` 같은 탭 콜백 화면에서 code/state를 검증하고, Native는 AuthSession 콜백을 사용한다.
...
- [ ] 앱 설정 화면
- [x] "인지 활동 상태" 카드 하단에 진단이 아닌 참고 정보임을 알리는 안내 문구 추가 (#136)
- [x] 일기 탭 카드가 목록 `preview`(80자) 대신 `GET /diaries/{diaryId}` 전문을 표시 (#136)
- [x] 일기 탭을 Figma 프로토타입 형식으로 정리 — ‹ › 월 이동, 선택일 📖 강조, 연한 초록 일기 카드(날짜·기분·전체 보기), 기분 범례 (#136)
...
- [ ] 다크 모드·글씨 크기의 **실제 적용** — `PATCH /users/{id}/preferences`에 해당 필드가
      없어 저장할 곳이 아직 없다
- [x] "피보호자 현황" 점수 카드 아래 참고용 스크리닝 결과 안내 문구 추가 — 행동 유도는 경보 카드가 담당 (#136)
...
- [x] 상담 기관 카드에 카카오맵 링크 추가 — 좌표 있으면 핀, 없으면 검색 링크 (#136)
- [x] 시/도 선택 시 "치매안심센터·신경과·보건소 찾기" 카카오맵 검색 링크 제공 — 등록 기관 없는 지역도 지도에서 탐색 (#136)
- [x] 정적 시/군/구 목록으로 지역 선택 후 `GET /counseling/nearby`(카카오 로컬) 결과를 등록 기관과 같은 카드로 표시, provider 미설정 시 검색 링크 폴백 (#138)
