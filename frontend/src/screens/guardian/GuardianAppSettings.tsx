 ...
 * 앱 설정 — 화면 설정과 앱 정보.
 *
 * Both preferences are local to this screen for now: `PATCH /users/{id}/preferences`
 * carries hearing, voice and notification settings but has no field for theme or
 * text scale, so there is nowhere to persist them yet. The note under each
 * control says so rather than leaving a switch that silently does nothing —
 * applying them is the follow-up work recorded on the Issue.
 */
const FONT_SIZES = [
      ...
      <SettingsGroup title="화면 설정">
        <View style={styles.row}>
          ...
          <Pressable
            ...
            style={[
              styles.switch,
              { backgroundColor: darkMode ? guardian.blue : colors.switchBackground },
            ]}
          >
            ...
            return (
              <Pressable
                ...
                style={[
                  styles.fontOption,
                  {
                    backgroundColor: on ? guardian.blueLight : "transparent",
              ...
              >
                <Text
                  style={{
                    ...
                    color: on ? guardian.blueDark : colors.foreground,
                  }}
                >
                  {size.label}
                </Text>
